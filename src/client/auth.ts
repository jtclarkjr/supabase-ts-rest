import { AuthTokenResponse, TokenRequestPayload, MagicLinkPayload, VerifyOTPPayload, DoRequestFn, AuthRequestFn } from '../types';
 ...
 */
export function createAuthMethods(doRequest: DoRequestFn, authRequest: AuthRequestFn) {
  return {
  /**
   ...
   * @returns The API response
   */
  async verifyOTP(email: string, tokenValue: string, otpType: string): Promise<unknown> {
      const payload: VerifyOTPPayload = { email, token: tokenValue, type: otpType };
